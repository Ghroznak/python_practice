...
def create_monster(dict):			#creates a new monster
	monster_attrib_values = (round(stats_roll(int) * difficulty, 0), round(stats_roll(int) * difficulty, 0),round(stats_roll(int) * difficulty, 0),round(stats_roll(int) * difficulty, 0),round(stats_roll(int) * difficulty, 0),round(stats_roll(int) * difficulty, 0))
...
def encounter():	#starts the fighting loop until Hero HP < 1
	...
	while hero_attributes['HP'] > 0 or monster_attributes['HP'] > 0:	#keeps looping the fighting as long as either hero or monster has > 0 HP
		...
		if attack_roll(hero_attributes['Str'], monster_attributes['Agi']) == True:	#checks if there is a hit
			...
			print ("ooo(:::::::> Monster Elf was hit for " + str(dmg) + " damage and is down to " + str(monster_attributes['HP']) + " hitpoints!")
		else:
			print ("MISS! Hero Orc missed!")
		time.sleep(2)
		...
		if attack_roll(monster_attributes['Str'], hero_attributes['Agi']) == True:
			...
			print ("ooo(:::::::> Hero Orc was hit for " + str(dmg) + " damage and is down to " + str(hero_attributes['HP']) + " hitpoints!")
		else:
